...
def ont_mac_learn(tn,PonID,OnuID,Ont_Port_ID):
    ...
    if result:
        ...
        return False
    #查看onu端口的mac地址
    tn.write(('display mac-address ont 0/1/%s %s eth %s '%(PonID, OnuID, Gemport_ID)).encode()+b'\n')
...
# mac_all=['0000-0022-2221', '0000-0022-2223', '0000-0022-2222', '0000-0022-2225', '0000-0022-2224']
# t_macs = []
# for t_mac in mac_all:
#     t_mac = ':'.join((':'.join((''.join((list(t_mac.split('-')[0])[0], list(t_mac.split('-')[0])[1])),
#                                 ''.join((list(t_mac.split('-')[0])[-2], list(t_mac.split('-')[0])[-1])))),

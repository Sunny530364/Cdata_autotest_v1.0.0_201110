...
def speedtest_test_wifi():
    try:
        ...
        # 判断SPEEDTEST的测试结果是否通过(判断条件上行和下行的速率大于60Mbps)
        if float(text1) > 1 or float(text2) > 1:
            cdata_info("SPEEDTEST测速成功")
            return True
        else:
            cdata_error("SPEEDTEST测速失败")
    ...
    for i in range(0, 6):
        ...
        while p.poll() is None:
            ...
            if line:
                ...
                if '的回复: 字节=32 时间=' in str(line, encoding='gbk'):
                    times = times +1
        print(times)
        if times == 4:
            print('ping成功了。')
    ...
    try:
        ...
        for interface in c.Win32_NetworkAdapterConfiguration(IPEnabled=True):
            if Network_car_name in interface.Caption:
                ...
                if len(interface.DNSServerSearchOrder) == 2:
...
def check_ip(Network_car_name):
    for i in range(0,6):
        ...
        for interface in c.Win32_NetworkAdapterConfiguration(IPEnabled=True):
            if Network_car_name in interface.Caption:
                print(interface)
                if "192.168.7" in interface.IPAddress[0]:
                    cdata_info("获取7网段的IP成功。")
                    ipv4_addr = interface.IPAddress[0]
                    default_gw = interface.DefaultIPGateway[0]
                    networkmask = interface.IPSubnet[0]
                    if len(interface.DNSServerSearchOrder) == 2:
        ...
        cdata_error('获取7网段的IP失败。')
        return False

def check_ip_1(Network_car_name):
    for i in range(0,6):
        time.sleep(1)
        c = wmi.WMI()
        # 获取电脑网卡DHCP开关状态，如果是关闭状态就开启
        ...
        for interface in c.Win32_NetworkAdapterConfiguration(IPEnabled=True):
            if Network_car_name in interface.Caption:
                print(interface)
                if "192.168.7" not in  interface.IPAddress[0]:
                    cdata_info("恢复LAN侧IP成功。")
                    ipv4_addr = interface.IPAddress[0]
                    default_gw = interface.DefaultIPGateway[0]
                    networkmask = interface.IPSubnet[0]
                    if len(interface.DNSServerSearchOrder) == 2:
...
def wifi_connect(wifi_ssid, wifi_password):
    ...
    profile.ssid=wifi_ssid   #wifi名称，貌似不能用中文？
    ...
    profile.key=wifi_password   #wifi密钥 如果无密码，则应该设置此项CIPHER_TYPE_NONE
...
def wifi_disconnect():
    ...
    for i in range(0, 3):
        ...
        print("=======================================")
        cdata_error("WIFI断开失败。")
        print("=======================================")
        return False

def disable_interface(interface_name):
    command = 'netsh interface set interface \"%s\" disable' % (interface_name)
    print(command)
...
def enable_interface(interface_name):
    command = 'netsh interface set interface \"%s\" enable' % (interface_name)
    print(command)
    result = os.system(command)

...
def ont_native_vlan(tn, PonID, OnuID, Ont_Port_ID, User_vlan):
    ...
    if ttt(PonID, OnuID) + '%s    enable   auto   auto   on     off     %s' % (
            Ont_Port_ID, User_vlan) in command_result:
        cdata_info("ONT 端口%s Native-vlan %s 配置成功"%(Ont_Port_ID,User_vlan))
        tn.write(b"exit" + b"\n")
        result = tn.read_until(b"OLT(config)#", timeout=2)
        return True
    else:
        cdata_info("==========================================")
        cdata_error("===============ERROR!!!===================")
        cdata_error("ONT 端口%s Native-vlan %s 配置失败"%(Ont_Port_ID,User_vlan))
        cdata_info("==========================================")
        tn.write(b"exit" + b"\n")
